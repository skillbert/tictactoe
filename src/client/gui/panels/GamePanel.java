...
public class GamePanel extends Panel {
	...
	private static int dimension = Protocol.DIM;
	
	public GamePanel(Gui gui) {
		this.gui = gui;
		this.whoseTurn = new JLabel();
		this.buttonLayout = new GridLayout(dimension, dimension);
		buttonPanel = new JPanel(buttonLayout);
		for (int row = 0; row < dimension; row++) {
			for (int col = 0; col < dimension; col++) {
				final Integer innerRow = new Integer(row); // workaround to
															// allow row and col
															// be used in a
															// thread (has to be
															// final)
	...
	public void refresh() {
		...
		for (JButton nextButton : buttonsl) {
			Point location = buttons.get(nextButton);
			String str = "";
			for (int height = 0; height < dimension; height++) {
				str += Mark
						.getMarkString(board.getField(board.index(location.y, location.x, height)));
			}
