...
public class Tui implements Ui {
	...
	public Tui(Session session) {
		...
		commands.put("connect", new CommandData(2, "<host> [port]", SessionState.disconnected,
				"Already connected", "handleConnect"));
		commands.put("login", new CommandData(2, "<name>", SessionState.authenticating,
				"Not connected or already logged in", "handleLogin"));
		commands.put("queue", new CommandData(1, "", SessionState.lobby,
				"Already queued / Not in the lobby", "handleQueue"));
		commands.put("test", new CommandData(1, "", SessionState.disconnected, "Already connected",
				"handleTest"));
		commands.put("place",
				new CommandData(3, "<x> <y>", SessionState.ingame, "Not in a game", "handlePlace"));
	}
	...
	private void parseInput(String input) {
		...
		if (commands.containsKey(parts[0])) {
			...
			try {
				this.getClass().getMethod(commandData.handler, new Class[] {String[].class })
						.invoke(this, new Object[] {parts });
			} catch (Exception e) {
				out.println(e);
			}
